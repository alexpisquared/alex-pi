...
namespace Db.OneBase.Model
{
  public partial class OneBaseContext : DbContext
  {
    ...
    public OneBaseContext(DbContextOptions<OneBaseContext> options, IConfiguration config)
        : base(options)
    {
      _config = config;
    }
    ...
    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
      ...
      {
        optionsBuilder.UseSqlServer(_config["WebApiStuff:DbOneConStr"]); // for Azure one use this: OneBaseDbConStr (btw: never used since StaartUp() line 40++)
      }
    }
